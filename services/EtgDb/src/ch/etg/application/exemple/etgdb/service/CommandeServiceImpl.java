...
public class CommandeServiceImpl implements CommandeService {
    ...
    Page<Commande> result = this.wmGenericDao.search(queryFilters, pageable);

    for (Commande c  : result.getContent()) {
      addTotalInCommande(c, pageable);
    }
    ...
    public Page<Commande> findAll(String query, Pageable pageable) {
        LOGGER.debug("Finding all Commandes");
            Page<Commande> result =  this.wmGenericDao.searchByQuery(query, pageable);
    for (Commande c  : result.getContent()) {
      addTotalInCommande(c, pageable);
    }
  ...
  private void addTotalInCommande(Commande commande, Pageable pageable) {
    Page<LigneCommande> result = findAssociatedLigneCommandes(commande.getId(), pageable);
    BigDecimal total = BigDecimal.ZERO;
    for (LigneCommande r : result.getContent()) {
      BigDecimal subTotal = r.getArticle().getPrix().multiply(BigDecimal.valueOf(r.getNombreArticle()));
